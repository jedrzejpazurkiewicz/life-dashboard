...
type Trade = { date: Date; pnl: number | null }

const DAYS = ['Pn', 'Wt', 'Śr', 'Cz', 'Pt', 'Sb', 'Nd']

export default function DayOfWeekChart({ trades }: { trades: Trade[] }) {
  const closed = trades.filter(t => t.pnl !== null)

  const buckets = DAYS.map((label, i) => ({ day: label, idx: i, pnl: 0, count: 0 }))

  for (const t of closed) {
    const d = new Date(t.date).getDay() // 0=Nd, 1=Pn...6=Sb
    const idx = d === 0 ? 6 : d - 1
    buckets[idx].pnl += t.pnl ?? 0
    buckets[idx].count++
  }

  const data = buckets.map(b => ({ ...b, pnl: Math.round(b.pnl * 100) / 100 }))
